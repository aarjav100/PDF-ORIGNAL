  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      if (mode === "signup") {
        ...
        const { data } = await supabase.auth.signInWithPassword({ email: parsed.data.email, password: parsed.data.password });
        if (data.session) navigate({ to: "/dashboard" });
      } else {
  ...
  return (
    <div className="grid min-h-screen lg:grid-cols-2">
      ...
      <div className="relative hidden overflow-hidden bg-primary text-primary-foreground lg:block">
        ...
        <div className="absolute -right-20 top-1/4 h-96 w-96 rounded-full bg-accent/30 blur-3xl" aria-hidden />
        <div className="relative flex h-full flex-col justify-between p-12">
          ...
          <div>
            ...
            <h1 className="mt-3 font-display text-5xl font-bold leading-tight">
              The PDF workspace<br />for actual humans.
            </h1>
            <p className="mt-4 max-w-md text-primary-foreground/70">Convert, edit, and template your documents — all in one private workspace.</p>
          </div>
      ...
      <div className="flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-sm">
          ...
          <h2 className="font-display text-3xl font-bold">{mode === "signup" ? "Create your account" : "Welcome back"}</h2>
          <p className="mt-2 text-sm text-muted-foreground">
            {mode === "signup" ? "Start converting and editing PDFs in seconds." : "Sign in to continue to your workspace."}
          </p>
          ...
          <form onSubmit={onSubmit} className="mt-8 space-y-4">
            ...
            >
              <svg className="mr-2 h-4 w-4" viewBox="0 0 24 24"><path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.76h3.56c2.08-1.92 3.28-4.74 3.28-8.09Z"/><path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.56-2.76c-.99.66-2.25 1.05-3.72 1.05-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84A11 11 0 0 0 12 23Z"/><path fill="#FBBC05" d="M5.84 14.1A6.6 6.6 0 0 1 5.5 12c0-.73.13-1.44.34-2.1V7.06H2.18A11 11 0 0 0 1 12c0 1.77.42 3.45 1.18 4.94l3.66-2.84Z"/><path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.65l3.15-3.15C17.45 2.09 14.97 1 12 1A11 11 0 0 0 2.18 7.06l3.66 2.84C6.71 7.3 9.14 5.38 12 5.38Z"/></svg>
              Continue with Google
            </Button>
            <div className="relative my-2">
              <Separator />
              <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-background px-2 text-xs uppercase text-muted-foreground">or</span>
            </div>
