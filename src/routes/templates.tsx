    ...
    (async () => {
      ...
      try {
        ...
        const { data: signed } = await supabase.storage
          .from("documents")
          .createSignedUrl(conv.output_path, 60);
          ...
          <aside className="space-y-2">
            {templates.length === 0 ? (
              <Card className="border-dashed bg-transparent p-6 text-center text-sm text-muted-foreground">
                ...
                <p className="mt-2">
                  No templates yet. Create one or generate from a PDF in the dashboard.
                </p>
              ...
              templates.map((t) => (
                <button
                  key={t.id}
                  onClick={() => open(t)}
                  className={`group flex w-full items-center justify-between rounded-lg border p-3 text-left transition-colors ${active?.id === t.id ? "border-accent bg-accent/5" : "border-border bg-card hover:bg-secondary"}`}
                >
                  <div className="min-w-0">
                    <p className="truncate font-medium">{t.name}</p>
                    <p className="font-mono text-xs text-muted-foreground">
                      {t.content?.placeholders?.length || 0} placeholders
                    </p>
                  </div>
                  <button
                    onClick={(e) => {
            ...
            {active === null && !name ? (
              ...
              <>
                ...
                <div className="mt-4 space-y-2">
                  <Label htmlFor="tbody">
                    Body — use{" "}
                    <code className="rounded bg-muted px-1 font-mono text-xs">{`{{placeholder}}`}</code>{" "}
                    for editable spots
                  </Label>
                  <Textarea
                    id="tbody"
