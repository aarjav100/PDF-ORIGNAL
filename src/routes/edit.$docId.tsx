...
function buildParagraphBlock(page: number, lines: TextSpan[][], idx: number): ParagraphBlock {
  ...
  const lineHeight =
    lines.length > 1
      ? Math.abs(
          lines[0][0].pdfY + lines[0][0].pdfHeight - (lines[1][0].pdfY + lines[1][0].pdfHeight),
        )
...
function Editor() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const looksImage =
          /\.(png|jpe?g|webp|gif|bmp)$/.test(lower) || blob.type.startsWith("image/");
        ...
        for (let i = 1; i <= pdf.numPages; i++) {
          ...
          const styles =
            (tc as unknown as { styles: Record<string, { fontFamily?: string }> }).styles || {};
          ...
          for (const item of tc.items as Array<{
            ...
            const cx = pdfX * SCALE,
              cyTop = (pdfViewport.height - pdfY - fontHeight) * SCALE;
  ...
  const updateAnn = useCallback(
    <T extends Annotation>(id: string, patch: Partial<T>) => {
  ...
  const onPageMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (d.kind === "draw") {
      setAnnotations((a) =>
        a.map((an) =>
          an.id === d.id && an.type === "draw" ? { ...an, points: [...an.points, { x, y }] } : an,
        ),
      );
    } else if (d.kind === "rect") {
      const nx = Math.min(d.startX, x),
        ny = Math.min(d.startY, y);
      const nw = Math.abs(x - d.startX),
        nh = Math.abs(y - d.startY);
      setAnnotations((a) =>
        a.map((an) =>
          an.id === d.id && an.type === "highlight" ? { ...an, x: nx, y: ny, w: nw, h: nh } : an,
        ),
      );
    } else if (d.kind === "move") {
      setAnnotations((a) =>
        a.map((an) => {
          ...
          if (an.type === "text" || an.type === "highlight") {
            return {
              ...an,
              x: Math.max(0, Math.min(1 - an.w, x - d.offX)),
              y: Math.max(0, Math.min(1 - an.h, y - d.offY)),
            };
          }
          return an;
        }),
      ...
      setAnnotations((a) =>
        a.map((an) => {
          ...
          if (d.corner.includes("w")) {
            nw = Math.max(0.02, o.w - dx);
            nx = o.x + (o.w - nw);
          }
          if (d.corner.includes("n")) {
            nh = Math.max(0.02, o.h - dy);
            ny = o.y + (o.h - nh);
          }
    ...
    const onDown = (e: MouseEvent) => {
      ...
      if (editingBlockId) {
        ...
        const clickedInsideBlock =
          blockEl?.contains(t) || t.closest(`[data-block-id="${editingBlockId}"]`);
    ...
    async (download = true) => {
      ...
      try {
        ...
        const getFont = async (sf: StandardFonts) => {
          if (!fontCache[sf]) fontCache[sf] = await pdf.embedFont(sf);
          return fontCache[sf]!;
        };
          ...
          if (a.type === "highlight") {
            ...
            for (let i = 1; i < a.points.length; i++) {
              ...
              p.drawLine({
                ...
                thickness: a.strokeWidth * (pw / 900),
                color: hexToRgb(a.color),
              });
            }
            ...
            if (a.underline) {
              ...
              p.drawLine({
                start: { x: a.x * pw, y: yLine },
                end: { x: a.x * pw + tw, y: yLine },
            ...
            p.drawCircle({
              ...
              y: cy,
              size: 8,
              color: hexToRgb("#facc15"),
              borderColor: hexToRgb("#a16207"),
              borderWidth: 1.5,
            });
          }
        }
        ...
        if (download) {
          ...
          await supabase.storage
            .from("documents")
            .upload(path, out, { contentType: "application/pdf" });
          await supabase
            .from("documents")
            .insert({
              user_id: user.id,
    ...
    async (download = true) => {
      ...
      try {
        ...
        for (const a of pageAnns) {
          ...
          ctx.lineWidth = a.strokeWidth * (dim.width / 900);
          ctx.lineCap = "round";
          ctx.lineJoin = "round";
          ctx.beginPath();
          ctx.moveTo(a.points[0].x * dim.width, a.points[0].y * dim.height);
          for (let i = 1; i < a.points.length; i++)
            ctx.lineTo(a.points[i].x * dim.width, a.points[i].y * dim.height);
          ctx.stroke();
          ctx.restore();
        }
        // text
        for (const a of pageAnns) {
          ...
          if (a.underline) {
            ...
            ctx.beginPath();
            ctx.moveTo(a.x * dim.width, a.y * dim.height + size * 1.05);
            ctx.lineTo(a.x * dim.width + tw, a.y * dim.height + size * 1.05);
            ctx.stroke();
          }
          ctx.restore();
        }
        ...
        if (download) {
          ...
          await supabase
            ...
            .insert({
              user_id: user.id,
              filename: `edited-${filename.replace(/\.[^.]+$/, "")}.png`,
              storage_path: path,
  ...
  const editedCount = useMemo(
    () => paragraphBlocks.filter((b) => b.edited).length + annotations.length,
    [paragraphBlocks, annotations],
  );
            ...
            <div className="pointer-events-auto flex items-center gap-1.5 rounded-full border border-border bg-background/95 px-3 py-1.5 shadow-lg backdrop-blur">
              ...
              {HIGHLIGHT_COLORS.map((c) => (
                <button
                  ...
                  className={`h-5 w-5 rounded-full border ${highlightColor === c ? "ring-2 ring-primary" : "border-border"}`}
                  style={{ background: c }}
                />
              ))}
            </div>
          ...
          {tool === "text" && (
            <div className="pointer-events-auto flex items-center gap-2 rounded-full border border-border bg-background/95 px-3 py-1.5 shadow-lg backdrop-blur">
              <select
                ...
                onChange={(e) => setTextDefaults((d) => ({ ...d, fontFamily: e.target.value }))}
              ...
              <Input
                ...
                onChange={(e) =>
                  setTextDefaults((d) => ({ ...d, fontSize: Number(e.target.value) || 16 }))
                }
                className="h-7 w-14"
              />
              <input
                ...
                onChange={(e) => setTextDefaults((d) => ({ ...d, color: e.target.value }))}
                className="h-7 w-7 cursor-pointer rounded-full border border-border bg-transparent"
              />
            </div>
          )}
          ...
          {selectedAnn && selectedAnn.type === "text" && (
            <div className="pointer-events-auto flex flex-wrap items-center gap-1.5 rounded-full border border-primary/40 bg-background/95 px-3 py-1.5 shadow-lg backdrop-blur">
              <select
                ...
                onChange={(e) => updateAnn<TextAnn>(selectedAnn.id, { fontFamily: e.target.value })}
                className="h-7 rounded border border-border bg-background text-xs"
              >
                {FONT_FAMILIES.map((f) => (
              ...
              <Input
                ...
                min={6}
                max={144}
                value={Math.round(selectedAnn.fontSize)}
                onChange={(e) =>
                  updateAnn<TextAnn>(selectedAnn.id, {
                    fontSize: Number(e.target.value) || selectedAnn.fontSize,
                  })
                }
                className="h-7 w-14"
              />
              <Button
                size="sm"
                variant={selectedAnn.bold ? "default" : "outline"}
                className="h-7 w-7 p-0"
                onClick={() => updateAnn<TextAnn>(selectedAnn.id, { bold: !selectedAnn.bold })}
              >
                <Bold className="h-3.5 w-3.5" />
              </Button>
              <Button
                size="sm"
                variant={selectedAnn.italic ? "default" : "outline"}
                className="h-7 w-7 p-0"
                onClick={() => updateAnn<TextAnn>(selectedAnn.id, { italic: !selectedAnn.italic })}
              >
                <Italic className="h-3.5 w-3.5" />
              </Button>
              <Button
                size="sm"
                variant={selectedAnn.underline ? "default" : "outline"}
                className="h-7 w-7 p-0"
                onClick={() =>
                  updateAnn<TextAnn>(selectedAnn.id, { underline: !selectedAnn.underline })
                }
              >
              ...
              <input
                ...
                onChange={(e) => updateAnn<TextAnn>(selectedAnn.id, { color: e.target.value })}
                className="h-7 w-7 cursor-pointer rounded-full border border-border bg-transparent"
              />
              <Button
                size="sm"
          ...
          {selectedAnn && selectedAnn.type === "highlight" && (
            <div className="pointer-events-auto flex items-center gap-1.5 rounded-full border border-primary/40 bg-background/95 px-3 py-1.5 shadow-lg backdrop-blur">
              {HIGHLIGHT_COLORS.map((c) => (
                <button
                  key={c}
                  onClick={() => updateAnn<HighlightAnn>(selectedAnn.id, { color: c })}
                  className={`h-5 w-5 rounded-full border ${selectedAnn.color === c ? "ring-2 ring-primary" : "border-border"}`}
                  style={{ background: c }}
                />
              ))}
              <Button
          ...
          {selectedAnn && selectedAnn.type === "draw" && (
            <div className="pointer-events-auto flex items-center gap-2 rounded-full border border-primary/40 bg-background/95 px-3 py-1.5 shadow-lg backdrop-blur">
              <input
                type="color"
                value={selectedAnn.color}
                onChange={(e) => updateAnn<DrawAnn>(selectedAnn.id, { color: e.target.value })}
                className="h-7 w-7 cursor-pointer rounded-full border border-border bg-transparent"
              />
              <input
                type="range"
                ...
                onChange={(e) =>
                  updateAnn<DrawAnn>(selectedAnn.id, { strokeWidth: Number(e.target.value) })
                }
                className="h-1 w-24"
              />
              <Button
              ...
              return (
                <div className="pointer-events-auto flex flex-wrap items-center gap-2 rounded-full border border-border bg-background/95 px-3 py-1.5 shadow-lg backdrop-blur">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updateBlock(activeBlock.id, {
                        fontSize: Number(e.target.value) || activeBlock.fontSize,
                      })
                    }
                    className="h-7 w-16"
                  />
                  ...
                  <Button
                    ...
                    className="h-7 w-7 p-0"
                    onClick={() =>
                      updateBlock(activeBlock.id, { underline: !activeBlock.underline })
                    }
                  >
                    <UnderlineIcon className="h-3.5 w-3.5" />
                  </Button>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  maxWidth: 900,
                  cursor: tool === "select" ? "default" : toolCursor,
                }}
              >
                <img
                  src={src}
                ...
                {tool === "select" &&
                  paragraphBlocks
                    ...
                    .map((block) => {
                      ...
                      const topPct =
                        (((dim.pdfHeight - block.pdfTopY + effectiveShiftY) * yScale) /
                          dim.height) *
                        100;
                  ...
                  {pageAnns
                    ...
                    .map((a) => (
                      <rect
                        key={a.id}
                        data-ann-hit
                        x={(a as HighlightAnn).x}
                        y={(a as HighlightAnn).y}
                        width={(a as HighlightAnn).w}
                        height={(a as HighlightAnn).h}
                        fill={(a as HighlightAnn).color}
                        opacity={0.35}
                        style={{
                ...
                {selectedAnn && selectedAnn.page === i && selectedAnn.type === "highlight" && (
                  <div
                    ...
                    style={{
                      left: `${selectedAnn.x * 100}%`,
                      top: `${selectedAnn.y * 100}%`,
                      width: `${selectedAnn.w * 100}%`,
                      height: `${selectedAnn.h * 100}%`,
                    }}
                  />
                )}
                {selectedAnn &&
                  ...
                  .map((a) => {
                    ...
                    return (
                      <div
                        ...
                        onMouseDown={(e) => {
                          ...
                          const onMove = (ev: MouseEvent) => {
                            ...
                            setAnnotations((arr) =>
                              arr.map((an) =>
                                an.id === ta.id && an.type === "text"
                                  ? {
                                      ...an,
                                      x: Math.max(0, Math.min(1 - an.w, x - (px - ta.x))),
                                      y: Math.max(0, Math.min(1 - an.h, y - (py - ta.y))),
                                    }
                                  : an,
                              ),
                            );
                        ...
                        onInput={(e) => {
                          ...
                          setAnnotations((arr) =>
                            arr.map((an) =>
                              an.id === ta.id && an.type === "text" ? { ...an, text: v } : an,
                            ),
                          );
                        }}
                        onBlur={() => {
                      ...
                      >
                        {isEditing
                          ? null
                          : ta.text || (isSelected ? "" : <span className="opacity-50">Text</span>)}
                              ...
                              <span
                                ...
                                onMouseDown={(e) => {
                                  ...
                                  dragRef.current = {
                                    ...
                                    startX: sx,
                                    startY: sy,
                                    orig: { x: ta.x, y: ta.y, w: ta.w, h: ta.h },
                                  };
                                  const onMove = (ev: MouseEvent) => {
                                    const x = (ev.clientX - rect.left) / rect.width;
                                    const y = (ev.clientY - rect.top) / rect.height;
                      ...
                      >
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            if (tool !== "select") return;
                            setSelectedAnnId(ca.id);
                            setActiveSpanId(null);
                            setEditingTextId(ca.id);
                          }}
                        ...
                        {(isSelected || isEditing) && (
                          <div
                            data-ann-hit
                            className="absolute left-8 top-0 z-20 w-56 rounded-lg border border-border bg-popover p-2 shadow-xl"
                          >
                            ...
                            <textarea
                              ...
                              onChange={(e) =>
                                updateAnn<CommentAnn>(ca.id, { text: e.target.value })
                              }
                              onMouseDown={(e) => e.stopPropagation()}
                ...
                {!isImage && (
                  <span className="absolute bottom-2 right-2 rounded bg-background/80 px-2 py-0.5 font-mono text-[10px] text-muted-foreground backdrop-blur">
                    Page {i + 1}
                  </span>
                )}
              </div>
