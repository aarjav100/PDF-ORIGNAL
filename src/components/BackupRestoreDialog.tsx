...
import { Cloud, CloudUpload, Download, ShieldCheck, Loader2, Upload, CheckCircle2, AlertCircle, FileJson, FileWarning } from "lucide-react";
...
export function BackupRestoreDialog({ open, onOpenChange, onRestored }: { open: boolean; onOpenChange: (o: boolean) => void; onRestored?: () => void }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    supabase
      .from("documents")
      .select("id,filename,storage_path,size_bytes,page_count,created_at,folder_id,is_favorite,last_opened_at")
      .eq("user_id", user.id)
      .is("deleted_at", null)
  ...
  const verifyBackup = async () => {
    if (!docs.length) { setVerifyResult({ ok: 0, missing: 0 }); return; }
    setVerifying(true);
    setVerifyResult(null);
    let ok = 0, missing = 0;
    for (const d of docs) {
      const { data, error } = await supabase.storage.from("documents").createSignedUrl(d.storage_path, 30);
      if (error || !data?.signedUrl) missing++; else ok++;
    }
  ...
  const onDrop = async (files: File[]) => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (existing.has(key)) { skipped++; setProgress({ done: i + 1, total: files.length }); continue; }
      ...
      const { error: upErr } = await supabase.storage.from("documents").upload(path, f, { contentType });
      if (!upErr) {
        await supabase.from("documents").insert({ filename: f.name, storage_path: path, size_bytes: f.size, user_id: user.id });
        added++;
      }
  ...
  const onManifestFile = async (file: File) => {
    ...
    try {
      ...
      if (!entries.length) { toast.error("Manifest contains no files"); setManifestBusy(false); return; }
      ...
      for (const e of entries) {
        ...
        if (existingKeys.has(`${e.filename}::${e.size_bytes}`) || existingPaths.has(e.storage_path)) { alreadyPresent++; continue; }
        const { data: signed, error: sErr } = await supabase.storage.from("documents").createSignedUrl(e.storage_path, 30);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2"><Cloud className="h-5 w-5" /> Backup & Restore</DialogTitle>
          <DialogDescription>Your files are continuously synced to secure cloud storage. Sign in on any device to restore instantly.</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div className="rounded-lg border border-border bg-card p-4">
            ...
            {loading ? (
              ...
              <div className="mt-3 grid grid-cols-3 gap-3 text-center">
                <div><div className="font-display text-xl font-bold">{totals.count}</div><div className="text-[10px] uppercase tracking-wider text-muted-foreground">Files</div></div>
                <div><div className="font-display text-xl font-bold">{fmt(totals.size)}</div><div className="text-[10px] uppercase tracking-wider text-muted-foreground">Total size</div></div>
                <div><div className="font-display text-xl font-bold">{totals.lastBackup ? new Date(totals.lastBackup).toLocaleDateString() : "—"}</div><div className="text-[10px] uppercase tracking-wider text-muted-foreground">Last sync</div></div>
              </div>
            )}
            {verifyResult && (
              <div className={`mt-3 flex items-center gap-2 rounded-md p-2 text-xs ${verifyResult.missing === 0 ? "bg-primary/10 text-primary" : "bg-destructive/10 text-destructive"}`}>
                {verifyResult.missing === 0 ? <CheckCircle2 className="h-3.5 w-3.5" /> : <AlertCircle className="h-3.5 w-3.5" />}
          ...
          <div className="grid grid-cols-2 gap-2">
            <Button variant="outline" size="sm" onClick={verifyBackup} disabled={verifying || loading}>
              {verifying ? <Loader2 className="mr-1.5 h-3.5 w-3.5 animate-spin" /> : <ShieldCheck className="mr-1.5 h-3.5 w-3.5" />}
              Verify backup
            </Button>
            <Button variant="outline" size="sm" onClick={downloadManifest} disabled={loading || !docs.length}>
              <Download className="mr-1.5 h-3.5 w-3.5" /> Export manifest
            </Button>
          </div>

          <div>
            <p className="mb-2 text-xs font-medium uppercase tracking-wider text-muted-foreground">Restore from device</p>
            <div
              {...getRootProps()}
            ...
            >
              ...
              {restoring && progress ? (
                <>
                  ...
                  <p className="text-sm font-medium">Restoring {progress.done} of {progress.total}…</p>
                </>
              ) : (
                <>
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">Duplicates (same name + size) are skipped automatically</p>
                </>
              )}
            </div>
          </div>
          ...
          <div>
            <p className="mb-2 text-xs font-medium uppercase tracking-wider text-muted-foreground">Restore from manifest</p>
            <div
            ...
            >
              ...
              {manifestBusy ? (
                <><Loader2 className="mb-2 h-5 w-5 animate-spin text-primary" /><p className="text-sm font-medium">Reading manifest…</p></>
              ) : (
                <><FileJson className="mb-2 h-5 w-5 text-muted-foreground" /><p className="text-sm font-medium">Drop a backup .json file, or click to browse</p><p className="mt-1 text-xs text-muted-foreground">Re-links files that still exist in cloud storage</p></>
              )}
            </div>
            {manifestResult && (
              <div className="mt-2 space-y-2 rounded-md border border-border bg-card p-2.5 text-xs">
                <div className="flex items-center gap-1.5 text-primary"><CheckCircle2 className="h-3.5 w-3.5" /> {manifestResult.recovered} recovered · {manifestResult.alreadyPresent} already present</div>
                {manifestResult.missing.length > 0 && (
                  <div className="space-y-1.5">
                    <div className="text-muted-foreground"><AlertCircle className="mr-1 inline h-3 w-3" />{manifestResult.missing.length} not in cloud — re-upload below: <span className="text-foreground">{manifestResult.missing.slice(0, 3).map((m) => m.filename).join(", ")}{manifestResult.missing.length > 3 ? ` +${manifestResult.missing.length - 3} more` : ""}</span></div>
                    <Button variant="outline" size="sm" className="h-7 gap-1.5 text-[11px]" onClick={downloadMissingReport}>
