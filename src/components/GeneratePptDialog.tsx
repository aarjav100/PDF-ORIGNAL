...
export function GeneratePptDialog({ documentId, filename, trigger }: Props) {
  ...
  const exportPptx = async () => {
    ...
    try {
      ...
      const safe =
        (deck.deckTitle || filename.replace(/\.pdf$/i, ""))
    ...
    >
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Turn <span className="font-medium">{filename}</span> into a polished PowerPoint deck.
          </DialogDescription>
        </DialogHeader>
        ...
        {!deck && (
          <div className="space-y-4">
            <RadioGroup
              value={mode}
              onValueChange={(v) => setMode(v as "summary" | "detailed")}
              className="grid grid-cols-2 gap-3"
            >
              <Label
                htmlFor="m-sum"
                className={`cursor-pointer rounded-lg border-2 p-4 transition-colors ${mode === "summary" ? "border-accent bg-accent/5" : "border-border"}`}
              >
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  1 slide · whole document at a glance
                </p>
              </Label>
              <Label
                htmlFor="m-det"
                className={`cursor-pointer rounded-lg border-2 p-4 transition-colors ${mode === "detailed" ? "border-accent bg-accent/5" : "border-border"}`}
              >
